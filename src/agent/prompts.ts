...
- \`tags[]\` — categorization tags. Fields: \`{ ref, name, description? }\`. Creates tags via the DevRev API (\`tags.create\`) that can be applied to tickets, issues, and articles in the UI. Emit tags when the brief mentions categorization, labeling, or routing by topic (e.g. "billing", "onboarding", "bug", "feature-request"). Keep names short (1-3 words), lowercase-with-hyphens. Typical POC: 4-8 tags covering the main ticket categories.
- \`custom_stages[]\` — custom ticket/issue lifecycle stages. Fields: \`{ ref, name, description?, state, ordinal }\`. \`state\` must be one of \`"open"\`, \`"in_progress"\`, or \`"closed"\`. \`ordinal\` is a 0-based position in the pipeline. Creates stages via \`stages.custom.create\`. Emit when the brief mentions a custom workflow, stage pipeline, or non-default ticket lifecycle. Example for a support pipeline: \`[{ name: "Triage", state: "open", ordinal: 0 }, { name: "Investigating", state: "in_progress", ordinal: 1 }, { name: "Waiting on Customer", state: "in_progress", ordinal: 2 }, { name: "Resolved", state: "closed", ordinal: 3 }]\`.
- \`groups[]\` — support teams and routing groups. Fields: \`{ ref, name, description?, members? }\`. Creates groups via \`groups.create\`. Emit when the brief mentions team structure, routing, or tiered support (e.g. "L1 Support", "Escalation Team", "Billing Specialists"). \`members\` is optional — if omitted, the group is created empty and the SE adds members in the UI.
- \`ui_guidance[]\` — free-form manual steps for things the templated primitives DON'T cover (saved views, workflow automations beyond feature_request_handler, multilingual config, etc.). **DO NOT write ui_guidance blocks for SLA policies, Slack, Feature Request Handler, PLuG, email channels, tags, custom stages, groups, or any listed integration** — those are auto-rendered from their respective blueprint sections. Duplicating them produces redundant sections in the plan. Cite https://devrev.ai/docs/DevRevU/demos and/or https://developer.devrev.ai/beta/about/for-developers. Don't invent UI menu paths.
